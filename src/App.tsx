...
function App() {
  ...
  useEffect(() => {
    void queryClient.cancelQueries({ queryKey: ['finance-state'], exact: false })
    ...
    <>
      ...
      <AppShell page={page} settings={settings} onPageChange={setPage} onThemeChange={(theme) => void actions.updateSettings({ ...settings, theme })}>
        <Suspense fallback={<PageSkeleton />}>
          ...
          {page === 'export' && <ExportPage transactions={transactions} categories={categories} accounts={accountBalances} />}
          {page === 'admin' && (
            <Admin
              users={users}
              categories={categories}
