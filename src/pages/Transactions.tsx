import { Copy, Edit3, Plus, Pin, ReceiptText, Search, Trash2 } from 'lucide-react'
import { useMemo, useRef, useState } from 'react'
import type { AccountBalance, AppSettings, Category, Transaction, TransactionType } from '../types'
...
export function Transactions({
  ...
  const filtered = useMemo(() => {
    ...
    return transactions.filter((item) => {
      const category = categoryMap.get(item.categoryId)?.name.toLowerCase() ?? ''
      const account = accountMap.get(item.accountId)?.name.toLowerCase() ?? ''
      const toAccount = item.toAccountId ? accountMap.get(item.toAccountId)?.name.toLowerCase() ?? '' : ''
  ...
  const activeCount = Number(typeFilter !== 'all') + Number(scope !== 'all') + Number(Boolean(query.trim()))
  ...
  return (
    <>
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="ds-display mt-2 text-zinc-950 dark:text-zinc-50">Операции</h1>
          </div>
        ...
        <div className="relative">
          ...
          <Input
            className="h-12 rounded-full pl-10 text-[14px]"
            value={query}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <Chip active={typeFilter === 'expense'} onClick={() => setTypeFilter('expense')}>Расходы</Chip>
          <Chip active={typeFilter === 'income'} onClick={() => setTypeFilter('income')}>Доходы</Chip>
          <Chip active={typeFilter === 'transfer'} onClick={() => setTypeFilter('transfer')}>Переводы</Chip>
          <Chip active={scope === 'month'} onClick={() => setScope(scope === 'month' ? 'all' : 'month')}>Месяц</Chip>
          <Chip active={scope === 'categories'} onClick={() => setScope(scope === 'categories' ? 'all' : 'categories')}>Кат.</Chip>
          <Chip active={scope === 'accounts'} onClick={() => setScope(scope === 'accounts' ? 'all' : 'accounts')}>Счета</Chip>
        </div>

        <div className="flex items-center justify-between">
          <p className="ds-caption font-medium text-zinc-500">{visible.length} операций</p>
          {activeCount > 0 && (
            <button type="button" className="ds-caption font-medium text-zinc-500" onClick={resetFilters}>
              Сбросить
            </button>
          )}
        </div>
        ...
        <div className="space-y-6">
          {groups.length === 0 ? (
            <EmptyState icon={ReceiptText} title="Операций нет" description="Попробуйте другой фильтр или добавьте новую операцию." />
          ) : (
            groups.map((group) => (
              <section key={group.key} className="space-y-3">
                <div className="flex items-end justify-between">
                  <div>
                    <p className="ds-section-title text-zinc-950 dark:text-zinc-50">{group.label}</p>
                    <p className="ds-caption mt-1 text-zinc-500">{group.items.length} операций</p>
                  </div>
                  {group.key === groupByPin && <span className="ds-caption text-zinc-500">Закреплено</span>}
                </div>
                <div className="space-y-2">
                  {group.items.map((item) => (
                    <SwipeCard
                      ...
                      toAccount={item.toAccountId ? accountMap.get(item.toAccountId) : undefined}
                      category={categoryMap.get(item.categoryId)}
                      onEdit={() => openEdit(item)}
                      onDuplicate={() => void duplicateTransaction(item.id).then(() => { void tapHaptic('success'); notify('Операция скопирована') })}
                      onDelete={() => setConfirmId(item.id)}
                      onPin={() => {
                        setGroupByPin((current) => (current === item.id ? null : item.id))
                        void tapHaptic('selection')
                      }}
      ...
      <Button
        className="fixed bottom-[calc(5.9rem+env(safe-area-inset-bottom))] right-4 z-40 h-14 w-14 rounded-full shadow-[0_18px_40px_rgba(24,24,27,0.18)] lg:bottom-6"
...
function Chip({ active, children, onClick }: { active: boolean; children: string; onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
        className={cn(
        'motion-soft inline-flex h-10 items-center rounded-full px-3.5 text-[12px] font-medium',
        active
  ...
  return (
    <div className="animate-enter relative isolate overflow-hidden rounded-[1.6rem] ds-surface">
      <div className="absolute inset-y-0 right-0 z-10 flex w-80 items-stretch">
        <button className="flex-1 bg-zinc-100 text-[12px] font-medium text-zinc-700 dark:bg-zinc-900 dark:text-zinc-200" onClick={onPin}>
          <Pin className="mx-auto mb-1 h-4 w-4" />
          {pinned ? 'Открепить' : 'Закрепить'}
        </button>
        <button className="flex-1 bg-zinc-50 text-[12px] font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200" onClick={onDuplicate}>
          <Copy className="mx-auto mb-1 h-4 w-4" />
          Копия
        </button>
        <button className="flex-1 bg-zinc-50 text-[12px] font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200" onClick={onEdit}>
          <Edit3 className="mx-auto mb-1 h-4 w-4" />
          Изменить
        </button>
        <button className="flex-1 bg-zinc-950 text-[12px] font-medium text-white dark:bg-white dark:text-zinc-950" onClick={onDelete}>
      ...
      <div
        ...
        onPointerMove={(event) => {
          if (!dragging) return
          const next = Math.max(-216, Math.min(72, startOffset.current + (event.clientX - startX.current)))
          setOffset(next)
        }}
        onPointerUp={() => {
          setDragging(false)
          setOffset((current) => (current < -110 ? -216 : current > 48 ? 72 : 0))
        }}
        onPointerCancel={() => {
          setDragging(false)
          setOffset(0)
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[1.15rem] bg-zinc-100 dark:bg-zinc-900" style={{ color: account?.color ?? '#18181b' }}>
                {item.type === 'transfer' ? <AccountIcon account={account} className="h-6.5 w-6.5" /> : <CategoryIcon category={category} className="h-6.5 w-6.5" />}
              </span>
              <div className="min-w-0">
                <p className="truncate text-[14px] font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{item.description || 'Операция'}</p>
                <p className="mt-1 truncate text-[12px] leading-5 text-zinc-500">
                  {category?.name ?? 'Другое'}
                  {scope !== 'categories' ? ` · ${account?.name ?? 'Счет'}` : ''}
                  {item.toAccountId && toAccount ? ` → ${toAccount.name}` : ''}
                </p>
                <p className="mt-1 text-[12px] leading-5 text-zinc-500">{formatDate(item.date)}</p>
              </div>
            </div>
