import { ArrowDownLeft, ArrowRightLeft, ArrowUpRight, ChevronRight, Edit3, Plus, Settings, Trash2, WalletCards } from 'lucide-react'
...
import { accountIconLabels, accountIconNames, accountTypeLabels } from '../constants/accounts'
import { tapHaptic } from '../services/haptics'
import type { Account, AccountBalance, AppSettings, Category, Transaction } from '../types'
...
export function Accounts({
  ...
  addAccount: (account: Omit<Account, 'id' | 'createdAt' | 'updatedAt'>) => Promise<unknown>
  ...
  const [selectedId, setSelectedId] = useState<string | null>(accounts.find((account) => !account.archived)?.id ?? null)
  const [detailsOpen, setDetailsOpen] = useState(false)
  const [createOpen, setCreateOpen] = useState(false)
  const [settingsAccount, setSettingsAccount] = useState<AccountBalance | null>(null)
  const [settingsDraft, setSettingsDraft] = useState<AccountSettingsDraft | null>(null)
  const [settingsOpen, setSettingsOpen] = useState(false)
  const [settingsSaving, setSettingsSaving] = useState(false)
  const [settingsError, setSettingsError] = useState<string | null>(null)
  const [transferAccount, setTransferAccount] = useState<AccountBalance | null>(null)
  const [transferOpen, setTransferOpen] = useState(false)
  const [operationAccount, setOperationAccount] = useState<AccountBalance | null>(null)
  const [operationMode, setOperationMode] = useState<'income' | 'expense'>('income')
  const [operationOpen, setOperationOpen] = useState(false)
  const [confirm, setConfirm] = useState<{ type: 'archive' | 'delete'; id: string } | null>(null)
  ...
  const totalBalanceAccounts = useMemo(
    () => activeAccounts.filter((account) => account.includeInTotalBalance !== false),
    [activeAccounts],
  )
  ...
  const selectedAccount = activeAccounts.find((account) => account.id === selectedId) ?? activeAccounts[0] ?? null
  const totalBalance = useMemo(() => totalBalanceAccounts.reduce((sum, account) => sum + account.balance, 0), [totalBalanceAccounts])
  const portfolioTrend = useMemo(() => buildPortfolioHistory(totalBalanceAccounts, transactions), [totalBalanceAccounts, transactions])
  const balanceTrend = useMemo(() => buildTrend(totalBalanceAccounts, transactions), [totalBalanceAccounts, transactions])
  const assetStructure = useMemo(() => buildAssetStructure(displayAccounts.filter((account) => account.includeInTotalBalance !== false)), [displayAccounts])
  ...
  const selectedTransactions = useMemo(
    () =>
      selectedAccount
        ? transactions
            .filter((transaction) => transaction.accountId === selectedAccount.id || transaction.toAccountId === selectedAccount.id)
  ...
  function openTransfer(account: AccountBalance) {
    if (activeAccounts.filter((item) => item.id !== account.id).length === 0) {
  ...
  return (
    <>
      <div className="animate-float space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="ds-display mt-1 text-zinc-950 dark:text-zinc-50">Счета</h1>
          </div>
          <Button
            variant="secondary"
            size="icon"
          ...
          <div className="grid gap-4 p-5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <CardDescription>Все счета вместе и динамика за 30 дней.</CardDescription>
              </div>
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              <AnimatedNumber
                value={totalBalance}
                currency={displayAccounts[0]?.currency ?? activeAccounts[0]?.currency ?? 'USD'}
              ...
              <div className="rounded-full bg-zinc-100/80 px-3 py-2 text-[12px] font-medium text-zinc-500 dark:bg-zinc-900/80 dark:text-zinc-300">
                <span className={balanceTrend.delta >= 0 ? 'text-emerald-600' : 'text-rose-500'}>
                  {balanceTrend.delta >= 0 ? '+' : ''}
                  {formatMoney(Math.abs(balanceTrend.delta), displayAccounts[0]?.currency ?? activeAccounts[0]?.currency ?? 'USD')}
                </span>{' '}
                за 30 дней
              </div>
            </div>
            <div className="h-24 rounded-[1.5rem] bg-zinc-50/80 p-3 dark:bg-zinc-900/70">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={portfolioTrend} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
                  <defs>
                    <linearGradient id="portfolioFill" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={chartStroke} stopOpacity={0.18} />
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => formatMoney(Number(value), activeAccounts[0]?.currency ?? 'USD')}
                  />
                  <Area type="monotone" dataKey="balance" stroke={chartStroke} strokeWidth={2.4} fill="url(#portfolioFill)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="grid gap-4">
          {displayAccounts.length === 0 ? (
            <EmptyState icon={WalletCards} title="Добавьте первый счет" description="Создайте карту, наличные или накопления." />
          ) : (
            displayAccounts.map((account) => (
              <button
                ...
                className="group flex min-h-[96px] items-center justify-between gap-4 rounded-[2rem] border border-zinc-200/70 bg-white px-4 py-4 text-left shadow-[0_14px_34px_rgba(24,24,27,0.05)] transition-[transform,box-shadow,border-color,background-color] duration-300 hover:-translate-y-0.5 hover:shadow-[0_22px_54px_rgba(24,24,27,0.08)] active:scale-[0.99] dark:border-zinc-800/70 dark:bg-zinc-950"
              >
                <div className="flex min-w-0 items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="truncate text-[1.08rem] font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{account.name}</p>
                ...
                <div className="flex items-center gap-2 text-right">
                  <p className="text-[1.7rem] font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">
        ...
        <Card className="animate-pop">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <CardDescription>Доли по текущему балансу счетов.</CardDescription>
            </div>
            <div className="rounded-full bg-zinc-100 px-3 py-2 text-[12px] font-medium text-zinc-500 dark:bg-zinc-900 dark:text-zinc-300">
              ...
              assetStructure.map((item) => (
                <div key={item.id} className="space-y-2">
                  <div className="flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      <p className="truncate text-[14px] font-medium text-zinc-950 dark:text-zinc-50">{item.name}</p>
                      <p className="text-[12px] text-zinc-500">{item.typeLabel}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[14px] font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{item.percent}%</p>
                      <p className="text-[12px] text-zinc-500">{formatMoney(item.value, item.currency)}</p>
                    </div>
                  </div>
      ...
      <Modal
        ...
        description={selectedAccount ? accountTypeLabels[selectedAccount.type] : undefined}
          ...
          <div className="space-y-5">
            <section className="rounded-[2.25rem] bg-zinc-950 p-5 text-white shadow-[0_24px_60px_rgba(24,24,27,0.18)] dark:bg-white dark:text-zinc-950">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="grid h-16 w-16 place-items-center rounded-[1.6rem] bg-white/10 shadow-inner dark:bg-zinc-950/10" style={{ color: selectedAccount.color }}>
                    <AccountIcon account={selectedAccount} className="h-9 w-9" />
                  </div>
                  <div>
                    <p className="text-[14px] font-medium text-white/60 dark:text-zinc-500">{accountTypeLabels[selectedAccount.type]}</p>
                    <p className="mt-1 text-[1.35rem] font-semibold tracking-tight text-white dark:text-zinc-950">{selectedAccount.name}</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-6">
                <p className="ds-caption text-white/60 dark:text-zinc-500">Доступно</p>
                <AnimatedNumber
                  value={selectedAccount.balance}
                  currency={selectedAccount.currency}
                  className="mt-2 text-[3rem] font-semibold tracking-tight text-white dark:text-zinc-950"
                />
                <p className="mt-3 text-[13px] text-white/60 dark:text-zinc-500">Баланс обновляется вместе с каждой операцией.</p>
              </div>
            </section>
            ...
            <section className="ds-surface rounded-[2rem] p-4 sm:p-5">
              <div className="mb-3 flex items-end justify-between">
                <div>
                  <p className="ds-section-title text-zinc-950 dark:text-zinc-50">История</p>
                  <p className="ds-caption mt-1 text-zinc-500">Последние 6 месяцев</p>
                </div>
                <p className="ds-caption text-zinc-500">{selectedAccount.currency}</p>
              </div>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={selectedHistory} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
                    <XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={12} />
                    <YAxis tickLine={false} axisLine={false} fontSize={12} width={40} />
                    <Tooltip
                      contentStyle={{ borderRadius: 16, border: '1px solid #e4e4e7' }}
                      formatter={(value) => formatMoney(Number(value), selectedAccount.currency)}
                    />
            ...
            <section className="grid grid-cols-3 gap-2">
              <MiniStat label="Доход" value={formatMoney(selectedAccount.monthlyIncome, selectedAccount.currency)} />
              <MiniStat label="Расход" value={formatMoney(selectedAccount.monthlyExpense, selectedAccount.currency)} />
              <MiniStat label="Переводы" value={formatMoney(sumTransfers(selectedTransactions, selectedAccount.id), selectedAccount.currency)} />
            </section>

            <section className="grid grid-cols-2 gap-2">
              <ActionButton onClick={() => openTransfer(selectedAccount)} icon={ArrowRightLeft}>
                Перевод
              </ActionButton>
              <ActionButton onClick={() => openOperation(selectedAccount, 'income')} icon={ArrowDownLeft}>
                Пополнить
              </ActionButton>
              <ActionButton onClick={() => openOperation(selectedAccount, 'expense')} icon={ArrowUpRight}>
                Снять
              </ActionButton>
              <ActionButton onClick={() => openSettings(selectedAccount)} icon={Settings}>
                Настройки
              </ActionButton>
              <ActionButton onClick={() => openSettings(selectedAccount)} icon={Edit3}>
                Переименовать
              </ActionButton>
              <ActionButton destructive onClick={() => setConfirm({ type: 'delete', id: selectedAccount.id })} icon={Trash2}>
                Удалить
              </ActionButton>
            </section>
            ...
            <section className="space-y-3">
              <div>
                <p className="ds-section-title text-zinc-950 dark:text-zinc-50">Последние операции</p>
                <p className="ds-caption mt-1 text-zinc-500">Недавние движения по счету</p>
              </div>
              {selectedTransactions.length === 0 ? (
                <EmptyState icon={WalletCards} title="Пока пусто" description="У этого счета еще нет операций." />
              ) : (
                <div className="space-y-2">
                  {selectedTransactions.slice(0, 5).map((transaction) => (
                    ...
                    >
                      <div className="min-w-0">
                        <p className="truncate text-[14px] font-medium text-zinc-950 dark:text-zinc-50">{transaction.description || 'Операция'}</p>
                        <p className="ds-caption text-zinc-500">{formatDate(transaction.date)}</p>
                      </div>
      ...
      <AccountSettingsSheet
        open={settingsOpen && Boolean(settingsAccount) && Boolean(settingsDraft)}
        ...
        onSave={async () => {
          ...
          try {
            const impact = settingsAccount.balance - settingsAccount.startingBalance
        ...
        {operationAccount && (
          <TransactionForm
            ...
            onSubmit={async (transaction) => {
              ...
              void tapHaptic('success')
              notify(operationMode === 'income' ? 'Счет пополнен' : 'Средства списаны')
              setOperationOpen(false)
              setOperationAccount(null)
            }}
          />
      ...
      <ConfirmDialog
        ...
        title={confirm?.type === 'archive' ? 'Архивировать счет?' : 'Удалить счет?'}
        description={confirm?.type === 'archive' ? 'Счет будет скрыт из новых операций, но история сохранится.' : 'Операции будут перенесены на доступный счет или удалены, если других счетов нет.'}
        confirmLabel={confirm?.type === 'archive' ? 'Архивировать' : 'Удалить'}
        onClose={() => setConfirm(null)}
        onConfirm={() => {
          if (!confirm) return
          void (confirm.type === 'archive' ? archiveAccount(confirm.id) : deleteAccount(confirm.id)).then(() => {
            void tapHaptic(confirm.type === 'archive' ? 'selection' : 'warning')
            notify(confirm.type === 'archive' ? 'Счет архивирован' : 'Счет удален')
          })
        }}
      />
    </>
...
function CreateAccountModal({
  ...
  return (
    <Modal open={createOpen} title="Новый счет" description="Создайте карту, наличные или накопления." onClose={onCreateClose}>
      <AccountForm
        draft={draft}
        onChange={setDraft}
        onSubmit={async () => {
          if (!draft.name.trim()) return
          await addAccount({ ...draft, name: draft.name.trim() })
          void tapHaptic('success')
          notify('Счет создан')
          onCreateClose()
        }}
...
function AccountSettingsSheet({
  ...
  const colorOptions = ['#525252', '#64748b', '#7c7c80', '#8b5cf6', '#4b5563', '#71717a', '#9ca3af', '#6b7280']
  const monthlyDelta = account.monthlyIncome - account.monthlyExpense
  const recentTransactions = [...transactions]
    .filter((transaction) => transaction.accountId === account.id || transaction.toAccountId === account.id)
    .sort((a, b) => b.date.localeCompare(a.date))
  const latestTransaction = recentTransactions[0] ?? null

  return (
    <Modal open={open} hideHeader title="Настройки счета" description={account.name} onClose={onClose} className="sm:max-w-[620px] sm:rounded-[2.1rem] sm:p-0">
      <div className="flex max-h-[92dvh] min-h-[85dvh] flex-col overflow-hidden rounded-t-[2.15rem] bg-[#f4f4f6] sm:max-h-[88vh] sm:rounded-[2rem]">
        <div className="sticky top-0 z-20 border-b border-black/5 bg-[#f4f4f6]/96 px-5 pb-4 pt-3 backdrop-blur-xl sm:px-6">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="min-w-0 flex-1 text-center">
              ...
              <h2 className="mt-1 truncate text-[28px] font-medium tracking-tight text-zinc-950 sm:text-[32px]">{account.name}</h2>
              <p className="mt-1 text-[14px] text-zinc-500">{accountTypeLabels[account.type]}</p>
            </div>
          ...
          <div className="mx-auto flex w-full max-w-2xl flex-col gap-5">
            <section className="overflow-hidden rounded-[2.35rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)] sm:p-7">
              <div className="flex items-start justify-between gap-5">
                <div className="flex min-w-0 items-center gap-4">
                  <div className="grid h-[72px] w-[72px] shrink-0 place-items-center rounded-[1.8rem]" style={{ backgroundColor: `${draft.color}18`, color: draft.color }}>
                    <AccountIcon account={{ icon: draft.icon }} className="h-10 w-10" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[13px] text-zinc-500">{accountTypeLabels[draft.type]}</p>
                    <p className="mt-1 truncate text-[24px] font-medium tracking-tight text-zinc-950 sm:text-[26px]">{draft.name || 'Название счета'}</p>
                    <p className="mt-2 text-[13px] text-zinc-500">Баланс обновляется после сохранения изменений.</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[12px] text-zinc-500">Баланс</p>
                  <p className="mt-1 text-[36px] font-medium tracking-tight text-zinc-950 sm:text-[42px]">{formatMoney(draft.balance, draft.currency)}</p>
                </div>
              </div>
              <div className="mt-5 grid gap-3 rounded-[1.7rem] bg-zinc-50/80 p-4 sm:grid-cols-3">
                <StatPill label="За 30 дней" value={formatMoney(monthlyDelta, draft.currency)} tone={monthlyDelta >= 0 ? 'positive' : 'negative'} />
                <StatPill label="Текущий баланс" value={formatMoney(account.balance, account.currency)} />
                <StatPill label="Последняя операция" value={latestTransaction ? formatDate(latestTransaction.date) : 'Нет операций'} />
              </div>
            </section>
            ...
            <section className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-[13px] text-zinc-500">Операции по этому счету.</p>
              </div>
              <div className="grid gap-3 sm:grid-cols-3">
                <QuickActionPill icon={ArrowUpRight} label="Доход" onClick={onIncome} />
                <QuickActionPill icon={ArrowDownLeft} label="Расход" onClick={onExpense} />
                <QuickActionPill icon={ArrowRightLeft} label="Перевод" onClick={onTransfer} />
              </div>
            </section>
            ...
            <section className="overflow-hidden rounded-[2.35rem] border border-black/5 bg-white shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
              <div className="border-b border-black/5 px-5 py-5 sm:px-6">
                <p className="text-[20px] font-medium tracking-tight text-zinc-950">Настройки</p>
                <p className="mt-1 text-[13px] text-zinc-500">Сначала смотрите сводку, затем меняйте параметры счета.</p>
              </div>

              <div className="space-y-5 px-5 py-5 sm:px-6">
                <SettingField label="Название счета">
                  <TextInput value={draft.name} placeholder="Основная карта" onChange={(value) => onChange({ ...draft, name: value })} />
                </SettingField>

                <div className="grid gap-4 sm:grid-cols-2">
                  <SettingField label="Тип счета">
                    <SelectInput value={draft.type} onChange={(value) => onChange({ ...draft, type: value as Account['type'] })}>
                  ...
                  <SettingField label="Валюта">
                    <SelectInput value={draft.currency} onChange={(value) => onChange({ ...draft, currency: value as Account['currency'] })}>
                  ...
                  <SettingField label="Иконка">
                    <div className="flex flex-wrap gap-2">
                      {iconOptions.map((icon) => (
                        ...
                        >
                          <span className="grid h-8 w-8 place-items-center rounded-full bg-white/70 text-[15px]">{iconEmoji(icon)}</span>
                          <span className="whitespace-nowrap">{accountIconLabels[icon]}</span>
                        </button>
                      ))}
                    </div>
                  </SettingField>
                      ...
                      {colorOptions.map((color) => (
                        <button
                          ...
                          className={cn(
                            ...
                            draft.color === color ? 'border-zinc-950 ring-4 ring-zinc-950/10' : 'border-black/5',
                          )}
                ...
                <div className="grid gap-4 rounded-[1.8rem] bg-[#f5f5f7] p-4 sm:grid-cols-3">
                  <StatPill label="Доходы" value={formatMoney(account.monthlyIncome, draft.currency)} />
                  <StatPill label="Расходы" value={formatMoney(account.monthlyExpense, draft.currency)} />
                  <StatPill label="Последняя операция" value={latestTransaction ? formatDate(latestTransaction.date) : 'Нет операций'} />
                </div>
                ...
                <div className="rounded-[1.8rem] bg-[#f5f5f7] p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-[15px] font-medium text-zinc-950">Скрыть счет из общего баланса</p>
                      <p className="mt-1 text-[13px] text-zinc-500">Счет останется в приложении, но не попадет в общий итог.</p>
                    </div>
                    <Switch.Root
                      checked={draft.includeInTotalBalance}
                      onCheckedChange={(checked) => onChange({ ...draft, includeInTotalBalance: checked })}
                ...
                <div className="space-y-3 border-t border-black/5 pt-5">
                  <p className="text-[20px] font-medium tracking-tight text-zinc-950">Опасная зона</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Button variant="secondary" className="h-12 justify-between rounded-[1.25rem] bg-[#f5f5f7] shadow-none" onClick={() => void onArchive()}>
                      <span>Архивировать</span>
                      <span className="text-zinc-400">↗</span>
                    </Button>
                    <Button
                ...
                {error && <div className="rounded-[1.35rem] bg-rose-50 px-4 py-3 text-[14px] text-rose-700 dark:bg-rose-950/30 dark:text-rose-200">{error}</div>}
              </div>
            </section>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 border-t border-black/5 bg-[#f4f4f6]/96 px-5 pt-4 backdrop-blur-xl sm:px-6">
          <div className="flex items-center gap-3 pb-[calc(env(safe-area-inset-bottom)+16px)]">
            <Button variant="secondary" className="h-[54px] flex-1 rounded-[1.3rem] bg-white/80 shadow-none" onClick={onClose} disabled={saving}>
              Отмена
            </Button>
            <Button
              ...
              onClick={() => void onSave()}
              disabled={saving || !draft.name.trim() || !Number.isFinite(draft.balance)}
            >
...
function StatPill({
  ...
  const toneClass =
    tone === 'positive' ? 'text-emerald-600' : tone === 'negative' ? 'text-rose-500' : 'text-zinc-950'
  ...
  return (
    <div className="rounded-[1.45rem] bg-white px-4 py-3 shadow-[0_10px_24px_rgba(24,24,27,0.04)]">
      ...
      <p className={cn('mt-1 text-[14px] font-medium tracking-tight', toneClass)}>{value}</p>
    </div>
  )
}
  ...
  return (
    <Button
      ...
      className={[
        ...
        destructive ? 'text-rose-600 hover:bg-rose-500/10 hover:text-rose-600' : '',
      ].join(' ')}
    >
  ...
  return (
    ...
    >
      <Field label="Название">
        <Input value={draft.name} onChange={(event) => onChange({ ...draft, name: event.target.value })} placeholder="Основная карта" />
      </Field>
      <Field label="Тип">
        <Select value={draft.type} onChange={(event) => onChange({ ...draft, type: event.target.value as Account['type'] })}>
          {Object.entries(accountTypeLabels).map(([value, label]) => (
            <option key={value} value={value}>
              {label}
            </option>
      ...
      <Field label="Стартовый баланс">
        <Input type="number" value={draft.startingBalance} onChange={(event) => onChange({ ...draft, startingBalance: Number(event.target.value) })} />
      </Field>
      <Field label="Валюта">
        <Select value={draft.currency} onChange={(event) => onChange({ ...draft, currency: event.target.value as Account['currency'] })}>
          <option value="USD">USD</option>
          <option value="EUR">EUR</option>
          <option value="UAH">UAH</option>
        </Select>
      ...
      <Field label="Иконка">
        <Select value={draft.icon} onChange={(event) => onChange({ ...draft, icon: event.target.value })}>
      ...
      <Field label="Цвет">
        <Input type="color" className="p-1" value={draft.color} onChange={(event) => onChange({ ...draft, color: event.target.value })} />
      </Field>
      <div className="sm:col-span-2">
        <Button type="submit">{draft.name ? 'Сохранить' : 'Создать счет'}</Button>
      </div>
    </form>
  )
}
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.4rem] bg-zinc-50 px-3 py-3 text-left dark:bg-zinc-900">
      ...
      <p className="mt-1 text-[15px] font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{value}</p>
    </div>
  )
}
...
function AnimatedNumber({
  ...
  return <span className={className}>{formatMoney(displayValue, currency)}</span>
}
...
function buildTrend(accounts: AccountBalance[], transactions: Transaction[]) {
  ...
  const delta = transactions.reduce((sum, transaction) => {
    ...
    if (transaction.type === 'income') return key === thisMonthKey ? sum + transaction.amount : sum - transaction.amount
    if (transaction.type === 'expense') return key === thisMonthKey ? sum - transaction.amount : sum + transaction.amount
    return sum
  }, 0)
...
function buildPortfolioHistory(accounts: AccountBalance[], transactions: Transaction[]) {
  const now = new Date()
  ...
  return months.map((month) => ({
    ...
    balance: accounts.reduce((portfolio, account) => {
      const accountBalance = account.startingBalance + transactions.reduce((sum, transaction) => {
...
function buildAccountHistory(account: AccountBalance, transactions: Transaction[]) {
  const now = new Date()
  const months = Array.from({ length: 6 }, (_, index) => {
    const date = new Date(now.getFullYear(), now.getMonth() - (5 - index), 1)
    return {
  ...
  return months.map((month) => ({
    label: month.label,
    balance: account.startingBalance + transactions.reduce((sum, transaction) => {
      const transactionDate = new Date(transaction.date)
      if (Number.isNaN(transactionDate.getTime())) return sum
      const key = `${transactionDate.getFullYear()}-${String(transactionDate.getMonth() + 1).padStart(2, '0')}`
      if (key > month.key) return sum
...
function sumTransfers(transactions: Transaction[], accountId: string) {
  return transactions.reduce((sum, transaction) => {
    ...
    if (transaction.accountId === accountId || transaction.toAccountId === accountId) return sum + transaction.amount
    return sum
  }, 0)
}
...
function signedAmount(transaction: Transaction, accountId: string) {
  if (transaction.type === 'transfer') {
    if (transaction.accountId === accountId) return `-${formatMoney(transaction.amount, transaction.currency)}`
    if (transaction.toAccountId === accountId) return `+${formatMoney(transaction.amount, transaction.currency)}`
  }
  return `${transaction.type === 'expense' ? '-' : '+'}${formatMoney(transaction.amount, transaction.currency)}`
}
...
function buildAssetStructure(accounts: AccountBalance[]) {
  ...
  return accounts.map((account) => {
    const basis = total > 0 ? Math.max(account.balance, 0) / total : 1 / Math.max(accounts.length, 1)
    const percent = Math.max(1, Math.round((total > 0 ? basis * 100 : fallbackShare)))
    return {
      id: account.id,
      name: account.name,
