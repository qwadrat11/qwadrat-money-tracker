...
import { transactionBaseAmount } from "../utils/multicurrency";
import { useQuery } from "@tanstack/react-query";
import { loadCryptoPortfolio } from "../services/cryptoPortfolio";

type MetricKey = "balance" | "income" | "expenses" | "turnover";
...
const overviewBlocksOrderKey = "qwadrat_finance_tracker_overview_blocks_order";
const defaultOverviewBlockOrder: DashboardWidgetId[] = [
  "balance",
  "quick-actions",
  "expenses",
  "income",
...
export function Dashboard({
  ...
  const [quickActionMode, setQuickActionMode] = useState<"expense" | "income">(
    "expense"
  );
  const [quickActionSeed, setQuickActionSeed] = useState(0);
  const [editingBlocks, setEditingBlocks] = useState(false);
  const monthTransactions = useMemo(
    () => transactions.filter((item) => item.date.startsWith(currentMonthKey)),
    [transactions]
  );
  ...
  const cryptoAccount = activeAccounts.find(
    (account) => account.type === "crypto_portfolio"
  );
  ...
  const totalBalance = balanceAccounts.reduce(
    (acc, account) => acc + (account.baseBalance ?? 0),
    0
  );
  ...
  const balanceHistory = useMemo(
    () =>
      buildBalanceHistory(transactions, balanceAccounts, settings.baseCurrency),
    [transactions, balanceAccounts, settings.baseCurrency]
  );
  const byCategory = useMemo(
    ...
    [monthTransactions, categoryMap, settings.baseCurrency]
  );
  ...
  const openMetric = useCallback((next: MetricKey) => {
  ...
  const overviewBlockIds = useMemo(
    () => overviewBlocks.map((block) => block.id),
    [overviewBlocks]
  );
  const [overviewOrder, setOverviewOrder] = useState<DashboardWidgetId[]>(() =>
    loadOverviewOrder(overviewBlockIds)
  );
  const [activeBlockId, setActiveBlockId] = useState<DashboardWidgetId | null>(
    null
  );
  ...
  useEffect(() => {
    setOverviewOrder((current) =>
      normalizeOverviewOrder(current, overviewBlockIds)
    );
  }, [overviewBlockIds]);
  ...
  const visibleBlocks = useMemo(() => {
    ...
    return overviewOrder
      .map((id) => map.get(id))
      .filter(Boolean) as typeof overviewBlocks;
    ...
    <>
      <div className="mb-5 flex items-start justify-between gap-3 sm:mb-6">
        <div className="min-w-0">
          <h1 className="ds-display text-zinc-950 dark:text-zinc-50">
            {t("pages.overview")}
          </h1>
          <p className="ds-caption mt-3 text-zinc-500 dark:text-zinc-400">
            {`Спокойная сводка за ${monthLabel(currentMonthKey)}.`}
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      {editingBlocks && (
        <div className="motion-soft mb-4 flex items-center justify-between gap-3 rounded-[1.35rem] border border-zinc-200/70 bg-white/80 px-4 py-3 shadow-[0_12px_32px_rgba(24,24,27,0.05)] backdrop-blur-xl dark:border-zinc-800/70 dark:bg-zinc-950/70">
          <div>
            ...
            <p className="ds-caption text-zinc-500 dark:text-zinc-400">
      ...
      {cryptoAccount && (
        ...
        >
          <Card className="flex min-w-0 items-center justify-between gap-3 p-4 transition hover:-translate-y-0.5">
            <div className="flex min-w-0 items-center gap-3">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-zinc-100 dark:bg-zinc-900">
                <span className="text-lg">₿</span>
              </div>
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold">Криптопортфель</p>
                <div className="mt-1 flex min-w-0 items-center gap-2 text-xs text-zinc-500">
                  <span className="whitespace-nowrap">
                    {cryptoPortfolio.data
                      ? formatMoney(
                          cryptoPortfolio.data.summary.portfolioValue,
                          cryptoPortfolio.data.summary.quoteCurrency
                        )
                      : "Обновляем…"}
                  </span>
                  {cryptoPortfolio.data && (
                    <span
                      className={`whitespace-nowrap ${
                        cryptoPortfolio.data.summary.portfolioChange24h >= 0
                          ? "text-emerald-600"
                          : "text-rose-600"
                      }`}
                    >
            ...
            <div className="flex shrink-0 items-center gap-2">
              {cryptoPortfolio.data?.positions
                ...
                .map((position) =>
                  position.imageUrl ? (
                    <img
                      key={position.id}
                      className="h-6 w-6 rounded-full border-2 border-white dark:border-zinc-950"
              ...
              <span className="hidden text-xs text-zinc-500 sm:inline">
                {cryptoPortfolio.data?.summary.coinCount ?? 0} монет
              </span>
      ...
      <Modal
        ...
        title={
          quickActionMode === "expense" ? "Добавить расход" : "Добавить доход"
        }
...
function buildOverviewBlocks({
  ...
  byCategory: { name: string; color: string; value: number; share: number }[];
  recent: Transaction[];
  ...
  return [
          ...
          <Card className="animate-pop overflow-hidden">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 ds-section-title text-zinc-950 dark:text-zinc-50">
                  Общий баланс
                </p>
                <p className="mt-3 text-[44px] font-medium tracking-tight text-zinc-950 dark:text-zinc-50 sm:text-[56px]">
                  {formatMoney(totalBalance, settings.currency)}
                </p>
                <p className="mt-3 text-[15px] text-zinc-500 dark:text-zinc-400">
                  <span
                    className={
                      balanceTrend(balanceHistory) >= 0
                        ? "text-emerald-600"
                        : "text-rose-500"
                    }
                  >
                    {balanceTrend(balanceHistory) >= 0 ? "+" : "-"}
                    {formatMoney(
                      Math.abs(balanceTrend(balanceHistory)),
                      settings.currency
                    )}
                  </span>{" "}
                  за 30 дней
              ...
              <div className="w-full max-w-[360px] rounded-[1.7rem] bg-[#f5f5f7] p-4 dark:bg-zinc-900/70">
    ...
    },
    {
      ...
      render: () => (
        <Metric
          onClick={() => openMetric("expenses")}
          title="Расходы месяца"
          value={formatMoney(expenses, settings.currency)}
          icon={ArrowDownRight}
        />
      ),
    },
    ...
    },
    {
      ...
      render: () => (
        <Metric
          onClick={() => openMetric("turnover")}
          title="Оборот"
          value={formatMoney(turnover, settings.currency)}
          icon={WalletCards}
        />
      ),
    },
    {
      id: "net-result" as const,
      span: 1,
      render: () => (
        <Metric
          title="Чистый результат"
          value={formatSignedMoney(netResult, settings.currency)}
          icon={ArrowRightLeft}
        />
      ),
    },
        ...
        <Card className="animate-pop">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Добавляйте расход, доход или перевод в один тап.
              </CardDescription>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-2">
            <Button
              variant="secondary"
              className="h-14 flex-col items-center justify-center gap-1 rounded-[1.45rem] px-3 text-[13px] font-medium sm:flex-row sm:justify-start sm:px-4 sm:text-[15px]"
            ...
            >
              <ArrowRightLeft className="h-[18px] w-[18px] text-zinc-500" />
              <span>Перевод</span>
            </Button>
          </div>
        </Card>
            ...
            {recent.slice(0, 5).length > 0 ? (
              recent.slice(0, 5).map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex items-center justify-between gap-3 py-3 animate-enter"
                  >
                    <div className="flex min-w-0 items-center gap-3">
              ...
              <EmptyDetailState
                ...
                description="Добавьте первый расход или доход, чтобы увидеть историю."
              />
            )}
          </div>
        </Card>
          ...
          <div className="mt-5 space-y-4">
            {byCategory.length > 0 ? (
              ...
              <EmptyDetailState
                ...
                description="Добавьте первый расход, чтобы увидеть категории месяца."
              />
            )}
          </div>
        </Card>
...
function OverviewSortableItem({
  ...
  const style = {
    ...
    transition:
      transition ??
      "transform 220ms cubic-bezier(0.2, 0.8, 0.2, 1), box-shadow 180ms ease, opacity 180ms ease",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        editing &&
          "rounded-[1.7rem] border border-dashed border-zinc-300/80 bg-white/[0.01] p-0.5 dark:border-zinc-700/80"
      )}
      style={style}
    >
      {editing && (
...
function loadOverviewOrder(registry: DashboardWidgetId[]) {
  try {
    ...
    const valid = saved.filter((id): id is DashboardWidgetId =>
      registry.includes(id as DashboardWidgetId)
    );
    if (valid.length === 0) {
      localStorage.removeItem(overviewBlocksOrderKey);
      return buildDefaultOverviewOrder(registry);
...
function buildDefaultOverviewOrder(registry: DashboardWidgetId[]) {
  return [
    ...defaultOverviewBlockOrder.filter((id) => registry.includes(id)),
    ...registry.filter((id) => !defaultOverviewBlockOrder.includes(id)),
  ];
}

function normalizeOverviewOrder(
  saved: string[],
  registry: DashboardWidgetId[]
): DashboardWidgetId[] {
  const known = new Set(registry);
  const filtered = saved.filter((id): id is DashboardWidgetId =>
    known.has(id as DashboardWidgetId)
  );
  const base =
    filtered.length > 0 ? filtered : buildDefaultOverviewOrder(registry);
  const unique = [
    ...new Set(base.filter((id) => known.has(id))),
  ...
  const content = (
    <Card className="animate-pop w-full">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="rounded-[1.1rem] bg-zinc-100 p-3 text-zinc-700 transition active:scale-95 dark:bg-zinc-900 dark:text-zinc-200">
...
function OverviewBlockDetailSheet({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      className="sm:max-w-[860px] sm:rounded-[2.25rem] sm:p-0"
    >
        ...
        <div className="sticky top-0 z-20 border-b border-black/5 bg-[#f4f4f6]/96 px-5 pb-4 pt-3 backdrop-blur-xl sm:px-6">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="min-w-0 flex-1 text-center">
              ...
              <h2 className="mt-1 text-[30px] font-medium tracking-tight text-zinc-950 sm:text-[38px]">
                {detailMeta[blockId].title}
              </h2>
              <p className="mt-1 text-[14px] text-zinc-500 sm:text-[15px]">
                {detailMeta[blockId].subtitle(data)}
              </p>
            </div>
            <div className="w-11" />
          </div>
...
function buildOverviewDetailContent(
      ...
      <>
        <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)] sm:p-7">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="min-w-0">
              <p className="text-[13px] text-zinc-500">{data.monthLabel}</p>
              <p className="mt-2 text-[46px] font-medium tracking-tight text-zinc-950 sm:text-[56px]">
                {formatMoney(data.totalBalance, data.currency)}
              </p>
        ...
        <section className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <DetailAction
              icon={WalletCards}
              label="Открыть счета"
              onClick={data.onOpenAccounts}
            />
            <DetailAction
              icon={ArrowUpRight}
              label="Добавить доход"
              onClick={data.onOpenIncome}
            />
            <DetailAction
              icon={ArrowRightLeft}
              label="Перевод"
              onClick={data.onOpenTransfer}
            />
          </div>
        </section>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-[13px] text-zinc-500">
                Распределение общего баланса между активными счетами.
              </p>
            </div>
          </div>
          <div className="mt-5 space-y-4">
            {accountShares(data.accounts).map((item) => (
              <div key={item.id} className="space-y-2">
                <div className="flex items-center justify-between gap-3 text-[14px]">
                  <div className="flex min-w-0 items-center gap-3">
                    <span
                      className="grid h-10 w-10 shrink-0 place-items-center rounded-[1.1rem]"
                      style={{
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-[15px] text-zinc-950">
                        {item.name}
                      </p>
                      <p className="text-[12px] text-zinc-500">
        ...
        <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-[13px] text-zinc-500">
                Скоро можно открыть полную историю.
              </p>
            </div>
            <Button
              variant="secondary"
              size="sm"
              className="rounded-full"
              onClick={data.onOpenTransactions}
            >
              Открыть операции
            </Button>
          </div>
          <div className="mt-4 divide-y divide-black/5">
            {data.recentTransactions.slice(0, 5).map((item) => (
              <DetailTransactionRow
                key={item.id}
                transaction={item}
                category={data.categoryMap.get(item.categoryId)}
                account={
                  data.accountMap.get(item.accountId) ?? data.accounts[0]
                }
            ...
            {data.recentTransactions.length === 0 && (
              <EmptyDetailState
                title="Операций пока нет"
                description="Добавьте первый расход или доход, чтобы увидеть движение по счетам."
              />
            )}
          </div>
        </section>
  ...
  if (blockId === "turnover") {
    return <TurnoverDetailContent data={data} />;
  }
                    ...
                    <div key={item.id} className="space-y-1.5">
                      <div className="flex items-center justify-between gap-3 text-[13px]">
                        ...
                        <span className="font-medium text-zinc-950">
                          {formatMoney(item.value, data.currency)}
                        </span>
                      </div>
        ...
        <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
          ...
          <div className="mt-5 space-y-4">
            ...
            {data.expenseTransactions.length === 0 && (
              <EmptyDetailState
                title="Расходов пока нет"
                description="Добавьте первый расход, чтобы увидеть категории."
              />
            )}
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
          ...
          <div className="mt-4 divide-y divide-black/5">
            ...
            {data.expenseTransactions.length === 0 && (
              <EmptyDetailState
                title="Расходов пока нет"
                description="Добавьте первый расход, чтобы увидеть историю."
              />
            )}
          </div>
        </section>
        ...
        <div className="mt-5 space-y-4">
          ...
          {data.incomeTransactions.length === 0 && (
            <EmptyDetailState
              ...
              description="Добавьте первый доход, чтобы увидеть источники."
            />
          )}
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-zinc-500">
              Самые свежие поступления месяца.
            </p>
          </div>
          <Button
            variant="secondary"
        ...
        <div className="mt-4 divide-y divide-black/5">
          {data.incomeTransactions.slice(0, 5).map((item) => (
            <DetailTransactionRow
              ...
              transaction={item}
              category={data.categoryMap.get(item.categoryId)}
              account={data.accountMap.get(item.accountId) ?? data.accounts[0]}
              currency={data.currency}
            />
          ))}
          {data.incomeTransactions.length === 0 && (
            <EmptyDetailState
              title="Доходов пока нет"
              description="Добавьте первый доход, чтобы увидеть историю."
            />
          )}
        </div>
      </section>
...
function TurnoverDetailContent({ data }: { data: OverviewDetailData }) {
  type TurnoverPeriod = "7d" | "30d" | "3m" | "1y";
  const [period, setPeriod] = useState<TurnoverPeriod>("30d");
  const currentPeriod =
    turnoverPeriods.find((item) => item.key === period) ?? turnoverPeriods[1];

  const periodState = useMemo(() => {
    const end = new Date();
    const start = new Date(end);
    start.setDate(end.getDate() - (currentPeriod.days - 1));
    start.setHours(0, 0, 0, 0);

    const withinRange = data.transactions.filter((transaction) => {
      const date = new Date(`${transaction.date.slice(0, 10)}T00:00:00`);
      return date >= start && date <= end;
    });

    const incomeTransactions = withinRange.filter(
      (transaction) => transaction.type === "income"
    );
    const expenseTransactions = withinRange.filter(
      (transaction) => transaction.type === "expense"
    );
    const days = Array.from({ length: currentPeriod.days }, (_, index) => {
      const date = new Date(start);
      date.setDate(start.getDate() + index);
      const key = date.toISOString().slice(0, 10);
      const income = sum(
        incomeTransactions.filter((item) => item.date.slice(0, 10) === key),
        data.currency
      );
      const expenses = sum(
        expenseTransactions.filter((item) => item.date.slice(0, 10) === key),
        data.currency
      );
      return {
        key,
    ...
    const totalIncome = sum(incomeTransactions, data.currency);
    const totalExpenses = sum(expenseTransactions, data.currency);
    const totalTurnover = totalIncome + totalExpenses;
    const netResult = totalIncome - totalExpenses;
    const uniqueExpenseDays =
      new Set(expenseTransactions.map((item) => item.date.slice(0, 10))).size ||
      1;
    const uniqueIncomeDays =
      new Set(incomeTransactions.map((item) => item.date.slice(0, 10))).size ||
      1;
    const largestPurchase = expenseTransactions.reduce<Transaction | null>(
      (best, item) =>
        !best ||
        (transactionBaseAmount(item, data.currency) ?? 0) >
          (transactionBaseAmount(best, data.currency) ?? 0)
          ? item
          : best,
      null
    );
    const mostProfitableDay = days.reduce(
    ...
    const operationsCount = withinRange.length;
    ...
    return {
      ...
      totalTurnover,
      netResult,
      avgExpense: totalExpenses / uniqueExpenseDays,
      avgIncome: totalIncome / uniqueIncomeDays,
      operationsCount,
      largestPurchase,
      mostProfitableDay,
  ...
  const cyclePeriod = useCallback(() => {
    const index = turnoverPeriods.findIndex((item) => item.key === period);
    const next = turnoverPeriods[(index + 1) % turnoverPeriods.length];
    setPeriod(next.key);
  }, [period]);
    ...
    <>
      <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)] sm:p-7">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div className="min-w-0">
            <p className="text-[13px] text-zinc-500">{data.monthLabel}</p>
            <h3 className="mt-2 text-[34px] font-medium tracking-tight text-zinc-950 sm:text-[42px]">
              Оборот
            </h3>
            <p className="mt-2 text-[46px] font-medium tracking-tight text-zinc-950 sm:text-[58px]">
              {formatMoney(periodState.totalTurnover, data.currency)}
            </p>
            <p className="mt-3 text-[15px] text-zinc-500">
              Доходы:{" "}
              <span className="text-emerald-600">
                +{formatMoney(periodState.totalIncome, data.currency)}
              </span>{" "}
              · Расходы:{" "}
              <span className="text-rose-500">
                -{formatMoney(periodState.totalExpenses, data.currency)}
              </span>{" "}
              · Чистый результат:{" "}
              <span
                className={
                  periodState.netResult >= 0
                    ? "text-emerald-600"
                    : "text-rose-500"
                }
              >
                {periodState.netResult >= 0 ? "+" : "-"}
                {formatMoney(Math.abs(periodState.netResult), data.currency)}
              </span>
            </p>
          </div>
          <div className="w-full max-w-[360px] rounded-[1.7rem] bg-[#f5f5f7] p-4">
            <p className="text-[13px] text-zinc-500">Период</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {turnoverPeriods.map((item) => (
                <button
                  key={item.key}
                  type="button"
                  className={cn(
                    ...
                    period === item.key
                      ? "bg-zinc-950 text-white shadow-[0_10px_24px_rgba(24,24,27,0.12)] dark:bg-white dark:text-zinc-950"
            ...
            <div className="mt-3 flex items-center justify-between text-[13px] text-zinc-500">
              <span>{periodState.periodLabel}</span>
              <span>{periodState.operationsCount} операций</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <DetailAction
            icon={GripVertical}
            label="Экспорт"
            onClick={() => data.onNavigate("export")}
          />
          <DetailAction
      ...
      <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-zinc-500">
              Плавная динамика денежных движений за выбранный период.
            </p>
          </div>
        </div>
      ...
      <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <MiniStat
            ...
            value={formatMoney(periodState.avgExpense, data.currency)}
          />
          <MiniStat
            label="Средний доход в день"
            value={formatMoney(periodState.avgIncome, data.currency)}
          />
          <MiniStat
            label="Количество операций"
            value={String(periodState.operationsCount)}
          />
          <MiniStat
            label="Самая крупная покупка"
            value={
              periodState.largestPurchase
                ? formatMoney(
          ...
          />
          <MiniStat
            label="Самый прибыльный день"
            value={periodState.mostProfitableDay?.label || "—"}
          />
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <Card className="animate-pop">
          ...
          <div className="mt-4 space-y-4">
            {periodState.topExpenseCategories.length > 0 ? (
              periodState.topExpenseCategories.map((item) => (
                <DetailProgressRow
                  key={item.id}
                  label={item.name}
                  value={item.value}
              ...
              <EmptyDetailState
                title="Расходов пока нет"
                description="За выбранный период нет категорий расходов."
              />
            )}
          </div>
        </Card>
        ...
        <Card className="animate-pop">
          <CardTitle>ТОП-5 категорий доходов</CardTitle>
          <div className="mt-4 space-y-4">
            {periodState.topIncomeCategories.length > 0 ? (
              periodState.topIncomeCategories.map((item) => (
                <DetailProgressRow
                  key={item.id}
                  label={item.name}
                  value={item.value}
              ...
              <EmptyDetailState
                title="Доходов пока нет"
                description="За выбранный период нет категорий доходов."
              />
            )}
          </div>
        </Card>
      ...
      <section className="overflow-hidden rounded-[2.25rem] border border-black/5 bg-white p-6 shadow-[0_22px_54px_rgba(24,24,27,0.07)]">
        ...
        <div className="mt-4 divide-y divide-black/5">
          ...
          {data.recentTransactions.length === 0 && (
            <EmptyDetailState
              title="Операций пока нет"
              description="Добавьте первый доход или расход, чтобы увидеть историю оборота."
            />
          )}
        </div>
      </section>
...
> = {
  balance: {
    ...
    subtitle: (data) =>
      `+${formatMoney(
        Math.abs(data.totalBalanceDelta),
        data.currency
      )} за 30 дней`,
  },
  expenses: {
    title: "Расходы месяца",
    subtitle: (data) =>
      `Текущий расход ${formatMoney(data.expenses, data.currency)}`,
  },
  income: {
    title: "Доходы месяца",
    subtitle: (data) =>
      `Текущий доход ${formatMoney(data.income, data.currency)}`,
  },
  turnover: {
    title: "Оборот",
    subtitle: (data) =>
      `${formatMoney(data.turnover, data.currency)} за ${data.monthLabel}`,
  },
};
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.55rem] bg-[#f5f5f7] p-4">
      ...
      <p className="mt-2 text-[15px] font-medium tracking-tight text-zinc-950">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-zinc-100">
        <div
          className="h-full rounded-full"
          style={{ width: `${Math.max(8, share)}%`, backgroundColor: color }}
        />
      </div>
    </div>
  );
...
function DetailTransactionRow({
  ...
  return (
    <button
      type="button"
      className="flex w-full items-center justify-between gap-4 py-4 text-left transition-colors hover:bg-zinc-50/80"
    >
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-zinc-500">
            {formatDate(transaction.date)} · {category?.name ?? "Категория"} ·{" "}
            {account?.name ?? "Счет"}
          </p>
        </div>
      </div>
      <span
        className={cn(
          "shrink-0 text-[15px] font-medium tracking-tight",
          transaction.type === "expense"
            ? "text-rose-500"
            : transaction.type === "income"
            ? "text-emerald-600"
...
function accountShares(accounts: AccountBalance[]) {
  const active = accounts.filter(
    (account) => !account.archived && account.includeInTotalBalance !== false
  );
  const total =
    active.reduce((sum, account) => sum + (account.baseBalance ?? 0), 0) || 1;
  return [...active]
    .sort((a, b) => b.balance - a.balance)
    .map((account) => ({
      ...account,
      share: Math.round(((account.baseBalance ?? 0) / total) * 100),
    }));
}
...
function categoryBreakdown(
  ...
  for (const item of transactions) {
    ...
    current.value += transactionBaseAmount(item, baseCurrency) ?? 0;
...
function sum(items: Transaction[], baseCurrency: AppSettings["baseCurrency"]) {
  return items.reduce(
    (acc, item) => acc + (transactionBaseAmount(item, baseCurrency) ?? 0),
    0
  );
}

function formatSignedMoney(value: number, currency: AppSettings["currency"]) {
...
function buildBalanceHistory(
  ...
  const series = Array.from({ length: 6 }, (_, index) => {
    ...
    const monthEnd = new Date(
      ...
      .slice(0, 10);
    const balance =
      accounts.reduce((acc, account) => acc + (account.baseBalance ?? 0), 0) +
      transactions.reduce((acc, transaction) => {
        if (transaction.date > monthEnd) return acc;
        const amount = transactionBaseAmount(transaction, baseCurrency);
        if (amount == null) return acc;
        if (transaction.type === "income") return acc + amount;
...
function balanceTrend(balanceHistory: { label: string; balance: number }[]) {
