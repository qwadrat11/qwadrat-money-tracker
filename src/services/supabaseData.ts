...
import { defaultCategories, defaultSettings } from "../data/demoData";
import type {
  Account,
  AppSettings,
  Category,
...
} from "./transactionCodec";
import { previewCurrencyConversion } from "./currencyConversion";
import { loadCryptoPortfolio } from "./cryptoPortfolio";
...
type TransactionRow = {
  ...
  exchange_rate_source?: "identity" | "NBU" | null;
...
function parseSettingsData(raw: unknown): SettingsData {
  ...
  const baseCurrency =
    source.baseCurrency ?? source.currency ?? defaultSettings.baseCurrency;
  return {
    ...defaultSettings,
    ...source,
    ...
    dashboardWidgetOrder: normalizeDashboardOrder(
      source.dashboardWidgetOrder ?? defaultSettings.dashboardWidgetOrder
    ),
  };
}
...
function normalizeDashboardOrder(order: string[]) {
  ...
  return [
    ...filtered,
    ...defaultOrder.filter((item) => !filtered.includes(item)),
  ];
}
...
function mapProfile(
  ...
  return {
    ...
    display_name:
      row?.display_name ?? email.split("@")[0] ?? "qwadrat Finance Tracker",
...
function mapCategory(row: CategoryRow): Category {
  return {
    ...
    isDefault: defaultCategories.some(
      (category) =>
        category.name === row.name &&
        normalizeCategoryType(category.type) === normalizeCategoryType(row.type)
    ),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
...
function mapTransaction(
  ...
  return {
    ...
    baseCurrency: row.base_currency ?? note.currency,
    exchangeRate:
      row.exchange_rate == null ? null : asNumber(row.exchange_rate),
    convertedAmount:
      row.converted_amount == null ? null : asNumber(row.converted_amount),
    exchangeRateDate: row.exchange_rate_date ?? null,
    exchangeRateSource: row.exchange_rate_source ?? null,
    destinationAmount:
      row.destination_amount == null
        ? undefined
        : asNumber(row.destination_amount),
...
function mapBudget(row: BudgetRow | null | undefined) {
  return {
    ...
    limitAmount: asNumber(row?.limit_amount ?? 0),
  };
}
...
async function ensureProfile(userId: string, email: string) {
  ...
  if (result.error) {
    throw toSupabaseError("Failed to upsert profile", result.error);
  }
}
...
async function ensureSeedData(userId: string, email: string) {
  ...
  if (profileResult.error) {
    throw toSupabaseError(
      "Failed to load profile seed probe",
      profileResult.error
    );
  }
  if (settingsResult.error) {
    throw toSupabaseError(
      "Failed to load app_settings seed probe",
  ...
  if (shouldSeedSettings) {
    ...
    if (result.error) {
      throw toSupabaseError("Failed to seed app_settings", result.error);
    }
  } else {
    const current = parseSettingsData(
      settingsResult.data?.[0]
        ? (settingsResult.data[0] as { data?: unknown }).data
        : {}
    );
    ...
    if (result.error) {
      throw toSupabaseError(
        "Failed to update app_settings seed data",
        result.error
      );
    }
  }
  ...
  return shouldSeedSettings || seededCategories;
}

function categoryKey(category: Pick<Category, "name" | "type">) {
  return `${category.name}::${normalizeCategoryType(category.type)}`;
}

const defaultCategoryTemplate = defaultCategories.map((category) => ({
  name: category.name,
...
async function ensureDefaultCategories(userId: string) {
  ...
  const { data, error } = await client
    ...
    .eq("user_id", userId);
  if (error) {
    throw toSupabaseError("Failed to inspect categories seed data", error);
  }

  const existing = new Set(
    (data ?? []).map((item) =>
      categoryKey({ name: item.name, type: normalizeCategoryType(item.type) })
    )
  );
  const missing = defaultCategoryTemplate.filter(
    (category) => !existing.has(categoryKey(category))
  );
  ...
  const { error: insertError } = await client.from("categories").insert(
    missing.map((category, index) => ({
      ...
      color: category.color,
      created_at: new Date(
        new Date(nowIso).getTime() + index * 1000
      ).toISOString(),
      updated_at: new Date(
        new Date(nowIso).getTime() + index * 1000
      ).toISOString(),
  ...
  if (insertError) {
    throw toSupabaseError("Failed to seed default categories", insertError);
  }
  return true;
}
...
async function fetchRawData(userId: string, email: string) {
  ...
  const rateByCurrency = new Map<Currency, number>([
    [settingsData.baseCurrency, 1],
  ]);
  await Promise.all(
    [...new Set(mappedAccounts.map((account) => account.currency))]
      .filter((code) => code !== settingsData.baseCurrency)
      .map(async (code) => {
        try {
          const snapshot = await previewCurrencyConversion({
            amount: 1,
            fromCurrency: code,
            toCurrency: settingsData.baseCurrency,
            date: currentDate,
          });
          rateByCurrency.set(code, snapshot.exchangeRate);
        } catch {
          // Missing current valuation is kept explicit; raw balances are never mixed.
        }
      })
  );
  for (const account of mappedAccounts) {
    if (account.type === "crypto_portfolio") {
      try {
        const snapshot = await loadCryptoPortfolio(account.id);
        account.baseBalance = snapshot.summary.portfolioValue;
        account.currency = snapshot.summary.quoteCurrency;
      } catch {
        account.baseBalance = undefined;
      }
      continue;
    }
    const rate = rateByCurrency.get(account.currency);
    account.baseBalance = rate == null ? undefined : account.balance * rate;
  }
  settingsData.monthlyBudget = budget.limitAmount;
  const users = Array.isArray(settingsData.users) ? settingsData.users : [];
  const receiptScans = Array.isArray(settingsData.receiptScans)
    ? settingsData.receiptScans
    : [];
...
export async function getBudget(month: string = currentMonth()) {
  ...
  const { data, error } = await client
    ...
    .maybeSingle();
  if (error) throw error;
  return data ? mapBudget(data) : { month, limitAmount: 0 };
}
...
export async function updateAccount(id: string, patch: Partial<Account>) {
  ...
  const next = {
    ...
    balance:
      ...
      asNumber((current.data as AccountRow).balance),
    ...
    is_archived:
      patch.archived ?? (current.data as AccountRow).is_archived ?? false,
    include_in_total:
      patch.includeInTotalBalance ??
      (current.data as AccountRow).include_in_total ??
      true,
...
export async function updateCategory(
  id: string,
  patch: Partial<Category> & { createdAt?: string }
) {
  const client = ensureSupabase();
  const user = await getAuthedUser();
  const current = await client
  ...
  const { data, error } = await client
    ...
    .update({
      ...
      type: normalizeCategoryType(
        patch.type ?? (current.data as CategoryRow).type
      ),
      icon:
        patch.icon ?? (current.data as CategoryRow).icon ?? "CircleEllipsis",
...
export async function deleteCategory(id: string) {
  ...
  const current = (categories.data ?? []).find((item) => item.id === id) as
    | CategoryRow
    | undefined;
  if (!current) return id;
  const fallback = (categories.data ?? []).find(
    (item) =>
      item.id !== id &&
      normalizeCategoryType((item as CategoryRow).type) ===
        normalizeCategoryType(current.type)
...
export async function deleteAccount(id: string) {
  ...
  const fallback = (accounts.data ?? []).find(
    (item) => item.id !== id && !(item as AccountRow).is_archived
  ) as AccountRow | undefined;
  ...
  if (fallback) {
    ...
    const accountChanges = transactions.filter(
      (item) => item.account_id === id || item.to_account_id === id
    );
    await Promise.all(
...
async function applyBalanceDeltas(
  ...
  for (const change of changes) {
    merged.set(
      change.accountId,
      (merged.get(change.accountId) ?? 0) + change.delta
    );
  }
  await Promise.all(
...
export async function createTransaction(input: TransactionInput) {
  ...
  const { data: response, error } = await client.functions.invoke(
    "transaction-save",
    { body: { action: "create", transaction: input } }
  );
  if (error) {
    throw new Error("Не удалось сохранить операцию с официальным курсом");
  }
  ...
  if (!result.success || !result.data) {
    throw new Error(result.error?.message ?? "Не удалось сохранить операцию");
  }
...
export async function updateTransaction(
  ...
  const merged: TransactionInput = {
    ...
    currency:
      patch.currency ??
      previous.currency ??
      decodeTransactionNote(previous.note).currency,
    createdAt: previous.created_at,
    updatedAt: now(),
  };
  const { data: response, error } = await client.functions.invoke(
    "transaction-save",
    { body: { action: "update", id, transaction: merged } }
  );
  if (error) {
    throw new Error("Не удалось обновить операцию с официальным курсом");
  }
  const result = response as {
    success?: boolean;
    data?: TransactionRow;
  ...
  if (!result.success || !result.data) {
    throw new Error(result.error?.message ?? "Не удалось обновить операцию");
  }
  const data = result.data;
  return mapTransaction(
    data as TransactionRow,
...
export async function deleteTransaction(id: string) {
  const client = ensureSupabase();
  const { data, error } = await client.functions.invoke("transaction-save", {
...
export async function updateSettings(input: AppSettings) {
  ...
  const payload = {
    ...
    dashboardWidgetOrder: normalizeDashboardOrder(
      input.dashboardWidgetOrder ?? defaultSettings.dashboardWidgetOrder
    ),
  };
  const { data, error } = await client
    .from("app_settings")
...
export async function importLegacyData(legacy: {
  ...
  const categories = legacy.categories.map((category, index) => {
    ...
    return {
      ...
      color: category.color,
      created_at: new Date(
        categoriesBase.getTime() + index * 1000
      ).toISOString(),
      updated_at: new Date(
        categoriesBase.getTime() + index * 1000
      ).toISOString(),
  ...
  await client.from("budgets").insert({
    ...
    month: currentMonth(),
    limit_amount:
      legacy.settings.monthlyBudget ?? defaultSettings.monthlyBudget,
  });
  ...
  const transactions = legacy.transactions.map((transaction) => ({
    ...
    user_id: user.id,
    account_id:
      accountIdMap.get(transaction.accountId) ?? accounts[0]?.id ?? null,
    category_id:
      categoryIdMap.get(transaction.categoryId) ?? categories[0]?.id ?? null,
    type: transaction.type,
...
export async function generateDemoData() {
  ...
  const expenseCategories = categories.filter(
    (category) => normalizeCategoryType(category.type) === "expense"
  );
  const incomeCategories = categories.filter(
    (category) => normalizeCategoryType(category.type) === "income"
  );
  ...
  for (let index = 0; index < 24; index += 1) {
    ...
    items.push({
      ...
      title: isIncome
        ? "Сгенерированный доход"
        : `Сгенерированный расход ${index + 1}`,
